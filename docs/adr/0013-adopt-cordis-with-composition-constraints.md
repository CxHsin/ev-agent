...
- Agent scope state and effects are owned by the Agent scope and must be isolated from sibling scopes; disposing one scope must not dispose another.
...
- A permanently unresolved `never` dependency failed without becoming current or leaving an effect.
- Normal unload, replacement, activation failure, and 1,000 load/unload cycles left zero tracked effects after cleanup.
- Two Agent scopes maintained independent state and effects; disposing one left the other active, and operations on the disposed scope failed explicitly.
